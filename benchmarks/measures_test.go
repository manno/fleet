...
func recordMemoryUsage(experiment *gm.Experiment, name string) {
	var m runtime.MemStats
	runtime.ReadMemStats(&m)
	experiment.RecordValue(name, float64(m.Alloc/1024/1024), gm.Style("{{red}}"), gm.Precision(0), gm.Units("MB"))
}

func recordResourceCount(experiment *gm.Experiment, name string) {
	n := 0
	...
	experiment.RecordValue(name, float64(n), gm.Style("{{red}}"), gm.Precision(0), gm.Units("resources"))
}

func recordCRDCount(setup *gm.Experiment, name string) {
...
func recordNodes(experiment *gm.Experiment) {
	...
	experiment.RecordNote(sb.String(), gm.Style("{{green}}"), gm.Annotation("Node Resources"))
	img := strings.Join(slices.Sorted(maps.Keys(images)), ", ")
	experiment.RecordNote(img, gm.Style("{{green}}"), gm.Annotation("Images"))
...
func recordMetrics(experiment *gm.Experiment, suffix string) {
	...
	for k, v := range res {
		...
		case "GCDuration", "CPU", "ReconcileTime", "WorkqueueQueueDuration", "WorkqueueWorkDuration":
			experiment.RecordValue(n, v, gm.Precision(0), gm.Units("seconds"))
